...
@pytest.fixture(scope="module")
def lesson_page(build_output):
    """Load a lesson page once for all tests."""
    with sync_playwright() as p:
        ...
        page.wait_for_selector('#status:has-text("Ready")', timeout=60000)
...
def test_error_shows_correct_line_number(lesson_page: Page):
    ...
    assert 'Line 3' in error_location
    # Make sure it's NOT showing a line from Pyodide internals (like 573)
    assert 'Line 573' not in error_location
...
def test_error_shows_hint(lesson_page: Page):
    ...
    assert 'missing_var' in hint or 'variable' in hint.lower()
...
def test_error_has_category_styling(lesson_page: Page):
    ...
    # Should have one of the category classes
    assert any(cat in class_name for cat in ['python-error', 'security-error', 'timeout-error', 'system-error'])
...
def test_banner_directs_to_output_tab(lesson_page: Page):
    ...
    assert 'output' in banner_text.lower() or 'Output' in banner_text
